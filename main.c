...
int get_region(int argc, char *argv[], struct seletion_box *select_region, struct screen_output *screens)
{
    ...
    if (screens != NULL)
    {
        struct slurp_output *output_t1;
        struct slurp_output *output_t;
        wl_list_for_each_safe(output_t1, output_t, &state.outputs, link)
        {
            if (screens->next == NULL)
            {
                screens->next = (struct screen_output *)malloc(sizeof(struct screen_output));
                screens = screens->next;
                screens->next = NULL;
            }
            screens->geometry.height = output_t1->geometry.height;
            screens->geometry.width = output_t1->geometry.width;
            screens->geometry.x = output_t1->geometry.x;
            screens->geometry.y = output_t1->geometry.y;
            screens->logical_geometry.height = output_t1->logical_geometry.height;
            screens->logical_geometry.width = output_t1->logical_geometry.width;
            screens->logical_geometry.x = output_t1->logical_geometry.x;
            screens->logical_geometry.y = output_t1->logical_geometry.y;
            screens->label = strdup(output_t1->logical_geometry.label);
            // printf("\n%d\n", output_t1->geometry.height);
            // printf("\n%p\n", output->surface);
        }
    }
    if (select_region == NULL)
    ...
    if (select_region != NULL)
    {
        select_region->geometry.height = state.result.height;
        select_region->geometry.width = state.result.width;
        select_region->geometry.x = state.result.x;
        select_region->geometry.y = state.result.y;
        struct slurp_output *output = output_from_box(&state.result, &state.outputs);
        if (output)
        {
            select_region->label = strdup(output->logical_geometry.label);
        }
    }
    // char *result_str = 0;
    // size_t length;
